...
try:
    ...
    def get_sync_config(): return {"auto_sync_enabled": False}
    def update_sync_config(updates): return True
    def toggle_auto_sync(enabled): return True
    def is_auto_sync_enabled(): return False
    def get_processing_stats(): return {"error": "Sync engine not available"}
...
def process_batch_files(file_paths: list):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to process batch files: {str(e)}")
...
def cleanup_old_files(days_to_keep: int = 7):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to cleanup old files: {str(e)}")
...
def get_watch_directory():
    ...
    try:
        ...
        return {
            ...
            "files": [f.name for f in watch_dir.iterdir() if f.is_file()] if watch_dir.exists() else []
        }
