 ...
 */
export const uploadCsvToServer = async (file, onProgress = null, endpoint = '/api/upload-csv') => {
  try {
    // Validate file first
 ...
 * @returns {Promise} Upload result
 */
export const uploadCsvWithFetch = async (file, endpoint = '/api/upload-csv') => {
  try {
    // Validate file first
    const validation = validateCsvFile(file)
    if (!validation.success) {
...
export const generateSampleCsv = () => {
  const headers = ['property_id', 'property_type', 'age_years', 'square_feet', 'contractor', 'maintenance_type', 'completion_time', 'cost', 'quality_score', 'monthly_cost']
