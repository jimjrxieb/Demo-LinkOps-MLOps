<template>
  <div class="p-6 animate-slide-in bg-gray-900 text-white min-h-screen">
    <div class="max-w-4xl mx-auto">
      <h2 class="text-3xl font-bold mb-6 text-blue-400">ML-Creator: Build Your ML Model</h2>
      <p class="mb-6 text-gray-300 text-lg">Easily create machine learning models without coding expertise. Perfect for non-data scientists.</p>
      
      <!-- Model Configuration Form -->
      <div class="bg-gray-800 rounded-lg p-6 mb-6 border border-gray-700">
        <h3 class="text-xl font-semibold mb-4 text-green-400">Model Configuration</h3>
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                class="w-full"
              />
              <span class="text-xs text-gray-400">{{ modelConfig.learningRate }}</span>
            </div>
            ...
            <div>
              ...
              <input
                ...
                class="w-full"
              />
              <span class="text-xs text-gray-400">{{ modelConfig.epochs }}</span>
            </div>
      ...
      <!-- Model Status -->
      <div v-if="modelStatus" class="bg-gray-800 rounded-lg p-6 border border-gray-700 animate-slide-in">
        ...
        <div class="bg-gray-700 rounded-lg p-4 mb-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <p class="font-semibold text-white">{{ modelStatus.modelName }}</p>
            </div>
            <div>
              <p class="text-sm text-gray-400">Status:</p>
              <p class="font-semibold text-green-400">{{ modelStatus.status }}</p>
            </div>
            <div>
              <p class="text-sm text-gray-400">Accuracy:</p>
              <p class="font-semibold text-blue-400">{{ modelStatus.accuracy }}%</p>
            </div>
            <div>
              <p class="text-sm text-gray-400">Training Time:</p>
              <p class="font-semibold text-yellow-400">{{ modelStatus.trainingTime }}</p>
            </div>
          </div>
        </div>
      ...
      <div v-if="recentModels.length > 0" class="mt-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          >
            ...
            <p class="text-sm text-gray-400 mb-2">{{ model.type }} - {{ model.dataset }}</p>
...
export default {
  ...
  computed: {
    isValidConfig() {
      return this.modelConfig.features.length > 0 && 
  ...
  methods: {
    generateModelCode() {
      return `from sklearn.${this.modelConfig.modelType.includes('RandomForest') ? 'ensemble' : 'linear_model'} import ${this.modelConfig.modelType}
...
print(f"Model accuracy: {accuracy:.2%}")`;
    ...
    async createModel() {
      ...
      this.modelStatus = {
        modelName: `${this.modelConfig.dataset}_${this.modelConfig.modelType.toLowerCase()}`,
