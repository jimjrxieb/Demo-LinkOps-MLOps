<template>
  <div class="p-6 animate-slide-in bg-gray-900 text-white min-h-screen">
    <div class="max-w-6xl mx-auto">
      <h2 class="text-3xl font-bold mb-6 text-blue-400">Demo Pipeline: Model Training</h2>
      <p class="mb-6 text-gray-300 text-lg">Experience a simplified, animated visualization of the Kubernetes ML model training pipeline.</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          >
            ...
            >
              ...
              <p class="text-xs text-gray-400">{{ stage.desc }}</p>
            ...
            <!-- Arrow connector -->
            <div v-if="index < stages.length - 1" class="hidden md:block absolute top-1/2 -right-2 transform -translate-y-1/2">
      ...
      <div class="bg-gray-800 rounded-lg p-6 mb-6 border border-gray-700">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-xl font-semibold text-blue-400">Current Stage: {{ stages[stage].name }}</h3>
        ...
        <div class="bg-gray-700 rounded-lg p-4 mb-4">
          <p class="text-gray-200 leading-relaxed">{{ stages[stage].desc }}</p>
        ...
        <div v-if="stages[stage].details" class="bg-gray-700 rounded-lg p-4">
          ...
          <ul class="text-sm text-gray-300 space-y-1">
            <li v-for="detail in stages[stage].details" :key="detail" class="flex items-center">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="bg-gray-800 rounded-lg p-4 border border-gray-700">
          ...
          <div class="text-2xl font-bold text-green-400">{{ Math.round((stage / (stages.length - 1)) * 100) }}%</div>
        </div>
        
        <div class="bg-gray-800 rounded-lg p-4 border border-gray-700">
          <h4 class="font-semibold text-blue-400 mb-2">Stages Completed</h4>
          <div class="text-2xl font-bold text-green-400">{{ stage }}/{{ stages.length - 1 }}</div>
        </div>
        
        <div class="bg-gray-800 rounded-lg p-4 border border-gray-700">
          <h4 class="font-semibold text-blue-400 mb-2">Estimated Time</h4>
          <div class="text-2xl font-bold text-green-400">{{ getEstimatedTime() }}</div>
        </div>
      </div>
    </div>
  </div>
