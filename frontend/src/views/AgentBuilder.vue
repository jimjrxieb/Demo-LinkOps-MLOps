<template>
  <div class="p-6 max-w-6xl mx-auto">
    ...
    <div class="mb-8">
      ...
      <p class="text-gray-600">Create AI agents and tools from natural language - fully offline, powered by local LLM</p>
    </div>
          ...
          <div class="card-body">
            ...
            <div class="stats stats-vertical lg:stats-horizontal shadow mb-4">
              <div class="stat">
                ...
                <div class="stat-value text-primary text-lg">{{ generatedTool.category }}</div>
              </div>
              <div class="stat">
                <div class="stat-title">Complexity</div>
                <div class="stat-value text-secondary">{{ generatedTool.estimated_complexity }}</div>
              </div>
            </div>
            ...
            <div class="form-control">
              ...
              <div class="mockup-code bg-base-300">
                <pre data-prefix="$"><code>{{ generatedTool.tool_code }}</code></pre>
              </div>
            </div>
            ...
            <div class="card-actions justify-end mt-4">
              <button @click="saveTool" class="btn btn-success">
                💾 Save Tool
              </button>
              ...
              >
                <div class="flex justify-between items-center">
                  <div>
                    ...
                    <span class="badge badge-primary ml-2">{{ info.file_extension }}</span>
                  </div>
                </div>
        ...
        <div class="card bg-base-100 shadow-xl">
          <div class="card-body">
            ...
            <div class="flex justify-between items-center mb-4">
              <button @click="loadTools" class="btn btn-outline">
                <span v-if="loadingTools" class="loading loading-spinner loading-sm"></span>
                {{ loadingTools ? 'Loading...' : 'Refresh Tools' }}
              </button>
              <span class="text-sm text-gray-500">{{ tools.length }} generated tools</span>
            </div>

            <div v-if="tools.length > 0" class="space-y-2 max-h-96 overflow-y-auto">
              <div 
                v-for="tool in tools" 
                :key="tool.tool_id"
              ...
              >
                <div class="flex justify-between items-center">
                  <div>
                    ...
                    <span class="badge ml-2" :class="getCategoryBadgeClass(tool.category)">
                      {{ tool.category }}
                    </span>
                  </div>
                  <div class="flex gap-2">
                    <span class="badge badge-outline">{{ tool.complexity }}</span>
                    <button 
                ...
                <div class="text-sm text-gray-500 mt-1">
                  {{ tool.lines_of_code }} lines • {{ formatFileSize(tool.file_size) }}
                </div>
              </div>
            </div>
            ...
            <div v-else class="text-center py-8 text-gray-500">
              ...
              <p>No tools generated yet. Describe a task to create your first tool!</p>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="showToolModal" class="modal modal-open">
      <div class="modal-box max-w-4xl">
        ...
        <div v-if="selectedTool" class="mb-4">
          <div class="stats stats-horizontal shadow">
            <div class="stat">
              ...
              <div class="stat-value text-primary text-lg">{{ selectedTool.tool_id }}</div>
            </div>
            <div class="stat">
              <div class="stat-title">Category</div>
              <div class="stat-value text-secondary">{{ selectedTool.category }}</div>
            </div>
            <div class="stat">
              <div class="stat-title">Complexity</div>
              <div class="stat-value">{{ selectedTool.complexity }}</div>
            </div>
          </div>
        </div>
        ...
        <div class="modal-action">
          <button @click="copySelectedTool" class="btn btn-outline">Copy Code</button>
          <button @click="closeToolModal" class="btn">Close</button>
        </div>
      </div>
    </div>
...
async function generateTool() {
  ...
  try {
    ...
    const response = await fetch('http://localhost:9000/agent-builder/agent/create', {
      method: 'POST',
      body: formData
    })
...
async function loadTools() {
  ...
  try {
    const response = await fetch('http://localhost:9000/agent-builder/agent/list')
    if (response.ok) {
...
async function loadCategories() {
  try {
    const response = await fetch('http://localhost:9000/agent-builder/categories')
    if (response.ok) {
      const data = await response.json()
...
async function deleteTool(toolId) {
  ...
  try {
    const response = await fetch(`http://localhost:9000/agent-builder/agent/${toolId}`, {
      method: 'DELETE'
    })
...
async function viewTool(tool) {
  try {
    const response = await fetch(`http://localhost:9000/agent-builder/agent/${tool.tool_id}`)
    if (response.ok) {
