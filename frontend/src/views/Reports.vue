...
const filteredModels = computed(() => {
  ...
  return models.value.filter(
    (model) => getModelType(model.name).toLowerCase() === modelFilter.value
  );
});
...
const filteredAgents = computed(() => {
  ...
  return agents.value.filter(
    (agent) => getAgentType(agent.name).toLowerCase() === agentFilter.value
  );
});
...
async function loadModels() {
  try {
    const response = await axios.get(
      'http://localhost:9000/model-creator/models'
    );
...
async function loadAgents() {
  try {
    const response = await axios.get(
      'http://localhost:9000/agent-creator/agents'
    );
...
async function downloadModel(modelName) {
  try {
    window.open(
      `http://localhost:9000/model-creator/download/${modelName}`,
      '_blank'
    );
  } catch (error) {
...
async function deleteModel(modelName) {
  ...
  try {
    await axios.delete(
      `http://localhost:9000/model-creator/models/${modelName}`
    );
    await loadModels();
    await loadStats();
  } catch (error) {
...
async function downloadAgent(agentName) {
  try {
    window.open(
      `http://localhost:9000/agent-creator/download/${agentName}`,
      '_blank'
    );
  } catch (error) {
...
async function deleteAgent(agentName) {
  ...
  try {
    await axios.delete(
      `http://localhost:9000/agent-creator/agents/${agentName}`
    );
    await loadAgents();
    await loadStats();
  } catch (error) {
...
async function downloadOutput(outputName) {
  try {
    window.open(
      `http://localhost:9000/pipeline/download/${outputName}`,
      '_blank'
    );
  } catch (error) {
...
async function viewDocument(docId) {
  try {
    const response = await axios.get(
      `http://localhost:9000/rag/documents/${docId}`
    );
