    ...
    <div class="bg-white shadow-sm border-b">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center py-6">
          <div>
            <h1 class="text-3xl font-bold text-gray-900">📓 Custom AI Keywords</h1>
            <p class="mt-1 text-sm text-gray-500">
              Teach the AI how your team talks. Add your own terms and what they mean.
            </p>
          </div>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-8">
        ...
        <p class="text-blue-800 mb-4">
          Add your property management terms and their variations. The AI will use these to better understand your questions and find relevant documents.
        </p>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-blue-700">
          <div>
            ...
            <ul class="list-disc list-inside mt-1 space-y-1">
              ...
              <li>Variations: "late rent", "rent not paid", "overdue payment"</li>
            </ul>
          </div>
          <div>
      ...
      <div class="bg-white rounded-lg shadow-sm border">
        <div class="p-6 border-b border-gray-200">
          <h2 class="text-xl font-semibold text-gray-900">Domain Terms & Variations</h2>
        ...
        <div class="p-6">
          <!-- Empty State -->
          <div v-if="terms.length === 0" class="text-center py-12 text-gray-500">
          ...
          <div v-else class="space-y-6">
            ...
            >
              <div class="flex items-start justify-between mb-4">
                <h3 class="text-lg font-medium text-gray-900">Term {{ index + 1 }}</h3>
                <button
              ...
              <div class="mb-4">
                ...
                <textarea
                  ...
                  @input="validateTerm(index)"
                ></textarea>
                <p v-if="term.errors.variations" class="mt-1 text-sm text-red-600">
                  {{ term.errors.variations }}
                </p>
                <p class="mt-1 text-xs text-gray-500">
                  Separate multiple variations with commas. These will be used as synonyms for the main term.
                </p>
              </div>
              ...
              <!-- Preview -->
              <div v-if="term.term && term.variations" class="mt-4 p-3 bg-gray-50 rounded-md">
                <h4 class="text-sm font-medium text-gray-700 mb-2">Preview:</h4>
                <p class="text-sm text-gray-600">
                  <strong>{{ term.term }}</strong> → 
                  <span class="text-indigo-600">{{ parseVariations(term.variations).join(', ') }}</span>
                </p>
              </div>
            </div>
          </div>
      ...
      <div class="mt-8 bg-white rounded-lg shadow-sm border p-6">
        <div class="flex items-center justify-between">
          <div>
            <h3 class="text-lg font-semibold text-gray-900">Save Keywords to AI</h3>
            <p class="text-sm text-gray-600 mt-1">
        ...
        <!-- Summary -->
        <div v-if="hasValidTerms" class="mt-4 p-4 bg-green-50 border border-green-200 rounded-md">
          <h4 class="text-sm font-medium text-green-900 mb-2">Ready to Save:</h4>
          <div class="text-sm text-green-800">
            <p><strong>{{ validTermsCount }}</strong> terms with <strong>{{ totalVariations }}</strong> variations</p>
            <p class="mt-1">These will be used to improve AI search and understanding of your domain.</p>
          </div>
        </div>
      </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <div class="text-sm text-gray-600 space-y-2">
              ...
              <p><strong>Misses:</strong> Documents about "delinquency" or "overdue payments"</p>
            </div>
          </div>
          <div>
            ...
            <div class="text-sm text-gray-600 space-y-2">
              <p><strong>User asks:</strong> "Who has late rent?"</p>
              <p><strong>AI searches for:</strong> "late rent", "delinquency", "overdue payments", "rent not paid"</p>
              <p><strong>Finds:</strong> All relevant documents regardless of terminology used</p>
            </div>
          </div>
        </div>
      </div>
...
const hasValidTerms = computed(() => {
  return terms.value.some(term => term.term && term.variations && !term.errors.term && !term.errors.variations)
})

const validTermsCount = computed(() => {
  return terms.value.filter(term => term.term && term.variations && !term.errors.term && !term.errors.variations).length
})
...
const parseVariations = (variations) => {
  ...
  return variations.split(',').map(v => v.trim()).filter(v => v.length > 0)
}
...
const saveTerms = async () => {
  try {
    ...
    // Filter valid terms
    const validTerms = terms.value.filter(term => 
      term.term && term.variations && !term.errors.term && !term.errors.variations
    )
