        ...
        <div class="flex justify-between items-center py-6">
          ...
          <div class="flex items-center space-x-4">
            <div class="text-sm text-gray-500">
              <span class="font-medium">{{ documentCount }}</span> documents indexed
            </div>
            <button
              ...
              :disabled="refreshing"
            >
              <svg v-if="refreshing" class="animate-spin -ml-1 mr-2 h-4 w-4 text-gray-500 inline" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <!-- Answer Display -->
          <div v-if="answer || loading" class="bg-white rounded-lg shadow-sm border">
            <div class="p-6">
              ...
              <div v-else-if="answer" class="space-y-4">
                ...
                <!-- Sources -->
                <div v-if="sources && sources.length > 0" class="mt-6 pt-4 border-t border-gray-200">
            ...
            <!-- Tenant Sources -->
            <div v-if="tenantSources.length > 0" class="mt-6 bg-white rounded-lg shadow-sm border p-6">
              <h4 class="text-lg font-semibold text-gray-900 mb-4">👥 Tenant Sources</h4>
              <div class="space-y-3">
                <div
                ...
                >
                  <div class="flex items-center justify-between mb-2">
                    <div class="flex items-center space-x-3">
                      <span class="text-sm font-medium text-gray-900">{{ source.tenant_name }}</span>
                      <span class="text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded">Unit {{ source.unit }}</span>
                      <span class="text-xs" :class="{
                        'bg-green-100 text-green-800': source.status === 'active',
                        'bg-red-100 text-red-800': source.status === 'inactive',
                        'bg-yellow-100 text-yellow-800': source.status === 'expired'
                      }">{{ source.status }}</span>
                    </div>
                  ...
                  <div class="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs text-gray-600">
                    <div>
                      <span class="font-medium">Lease End:</span> {{ formatDate(source.lease_end) }}
                    </div>
                    <div v-if="source.rent_amount">
                      <span class="font-medium">Rent:</span> ${{ formatCurrency(source.rent_amount) }}
                    </div>
                    <div>
                      <span class="font-medium">Source:</span> {{ source.source }}
                    </div>
                    <div>
          ...
          <!-- No Results -->
          <div v-else-if="searched && !answer" class="bg-white rounded-lg shadow-sm border p-6 text-center">
        ...
        <div class="space-y-6">
          ...
          <div class="bg-white rounded-lg shadow-sm border p-6">
            ...
            <DropZone @file-uploaded="onFileUploaded" @upload-error="onUploadError" />
          </div>

          <!-- Recent Queries -->
          <div v-if="recentQueries.length > 0" class="bg-white rounded-lg shadow-sm border p-6">
          ...
          <div class="bg-white rounded-lg shadow-sm border p-6">
            ...
            <div class="space-y-3">
              ...
              <div class="flex items-center justify-between">
                ...
                <span class="text-sm font-medium text-gray-900">{{ documentCount }}</span>
              </div>
              <div class="flex items-center justify-between">
                <span class="text-sm text-gray-600">Vector Store</span>
                <span class="text-sm font-medium text-gray-900">{{ vectorStoreSize }}</span>
              </div>
            </div>
          </div>
        </div>
  ...
  try {
    ...
    if (data.results && data.results.length > 0) {
      ...
      sources.value = data.results.map(result => ({
        ...
        document_name: result.metadata?.document_name || result.metadata?.source || 'Unknown',
    ...
    answer.value = 'Sorry, I encountered an error while processing your query. Please try again.'
  } finally {
    loading.value = false
  }
}
