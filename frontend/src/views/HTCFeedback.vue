    ...
    <div class="bg-white shadow-sm border-b">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center py-6">
          <div>
            <h1 class="text-3xl font-bold text-gray-900">🧠 AI Feedback & Corrections</h1>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div v-if="showFeedbackForm" class="mb-8">
        <div class="bg-white rounded-lg shadow-sm border p-6">
          <h2 class="text-lg font-semibold text-gray-900 mb-4">📝 Add Feedback</h2>
        ...
        <div class="bg-white rounded-lg shadow-sm border p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div class="text-center p-4 bg-blue-50 rounded-lg">
              <div class="text-2xl font-bold text-blue-600">{{ feedbackStats.total_feedback }}</div>
              <div class="text-sm text-gray-600">Total Feedback</div>
            </div>
            <div class="text-center p-4 bg-yellow-50 rounded-lg">
              <div class="text-2xl font-bold text-yellow-600">{{ feedbackStats.pending_training }}</div>
              <div class="text-sm text-gray-600">Pending Training</div>
            </div>
            <div class="text-center p-4 bg-green-50 rounded-lg">
              <div class="text-2xl font-bold text-green-600">{{ feedbackStats.trained }}</div>
              <div class="text-sm text-gray-600">Trained</div>
            </div>
            <div class="text-center p-4 bg-purple-50 rounded-lg">
              <div class="text-2xl font-bold text-purple-600">{{ feedbackStats.recent_feedback }}</div>
      ...
      <div class="space-y-6">
        <div class="flex items-center justify-between">
          <h3 class="text-lg font-semibold text-gray-900">📝 Recent Feedback Entries</h3>
        ...
        <div v-if="filteredFeedback.length === 0" class="text-center py-12 text-gray-500">
          ...
          >
            <div class="flex items-start justify-between mb-4">
              <div class="flex items-center space-x-3">
                <span class="text-sm font-medium text-gray-900">{{ entry.query }}</span>
                <span class="text-xs px-2 py-1 rounded-full" :class="{
                  'bg-red-100 text-red-800': entry.category === 'incorrect_answer',
                  'bg-yellow-100 text-yellow-800': entry.category === 'incomplete_answer',
                  'bg-blue-100 text-blue-800': entry.category === 'wrong_context',
                  'bg-purple-100 text-purple-800': entry.category === 'missing_information',
                  'bg-gray-100 text-gray-800': entry.category === 'unclear_response',
                  'bg-orange-100 text-orange-800': entry.category === 'other'
                }">
                  {{ formatCategory(entry.category) }}
                </span>
                <span class="text-xs px-2 py-1 rounded-full" :class="{
                  'bg-yellow-100 text-yellow-800': entry.status === 'pending_training',
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <!-- AI's Answer -->
              <div>
                <h4 class="text-sm font-medium text-gray-700 mb-2">🤖 AI's Answer</h4>
                <div class="p-3 bg-red-50 border border-red-200 rounded-md">
                  <p class="text-sm text-gray-700">{{ entry.generated_answer }}</p>
                </div>
              </div>
              ...
              <div>
                ...
                <div class="p-3 bg-green-50 border border-green-200 rounded-md">
                  <p class="text-sm text-gray-700">{{ entry.expected_answer }}</p>
                </div>
              </div>
            </div>

            <!-- Context and Notes -->
            <div v-if="entry.context || entry.user_notes" class="mt-4 space-y-2">
              <div v-if="entry.context">
                <h4 class="text-sm font-medium text-gray-700 mb-1">📄 Context</h4>
                <p class="text-sm text-gray-600">{{ entry.context }}</p>
              </div>
              <div v-if="entry.user_notes">
                <h4 class="text-sm font-medium text-gray-700 mb-1">💭 Notes</h4>
                <p class="text-sm text-gray-600">{{ entry.user_notes }}</p>
              </div>
            </div>
...
const filteredFeedback = computed(() => {
  ...
  return feedbackEntries.value.filter(entry => entry.category === filterCategory.value)
})
