    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="bg-white rounded-lg shadow p-6">
          <h2 class="text-lg font-medium text-gray-900 mb-6">Model Configuration</h2>
          ...
          <div class="mb-6">
            ...
            <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md hover:border-gray-400 transition-colors">
              <div class="space-y-1 text-center">
                ...
                <p class="text-xs text-gray-500">CSV, JSON, Excel files up to 10MB</p>
              </div>
            </div>
          ...
          <!-- Target Column -->
          <div class="mb-6" v-if="modelType !== 'clustering'">
            <label class="block text-sm font-medium text-gray-700 mb-2">
              Target Column
            </label>
            <input
          ...
          <div class="mb-6">
            ...
            <select
              ...
              class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm rounded-md"
            >
              <option v-if="modelType === 'classifier'" value="random_forest">Random Forest</option>
              <option v-if="modelType === 'classifier'" value="svm">Support Vector Machine</option>
              <option v-if="modelType === 'classifier'" value="neural_network">Neural Network</option>
              <option v-if="modelType === 'regression'" value="linear_regression">Linear Regression</option>
              <option v-if="modelType === 'regression'" value="random_forest">Random Forest</option>
              <option v-if="modelType === 'regression'" value="neural_network">Neural Network</option>
              <option v-if="modelType === 'clustering'" value="kmeans">K-Means</option>
              <option v-if="modelType === 'clustering'" value="dbscan">DBSCAN</option>
              <option v-if="modelType === 'time_series'" value="lstm">LSTM</option>
              <option v-if="modelType === 'time_series'" value="arima">ARIMA</option>
            </select>
          </div>
          ...
          <div v-if="generationResult" class="bg-white rounded-lg shadow p-6">
            ...
            <div class="space-y-4">
              ...
              <div class="flex items-center justify-between">
                ...
                <span class="text-sm text-gray-900">{{ generationResult.path }}</span>
              </div>
              <div class="flex items-center justify-between">
    ...
    <div v-if="isLoading" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
      <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3 text-center">
          ...
          <h3 class="text-lg leading-6 font-medium text-gray-900 mt-4">Generating Model</h3>
...
async function previewData(file) {
  try {
    ...
    const response = await axios.post('http://localhost:8002/preview', formData, {
      headers: {
...
async function generateModel() {
  ...
  try {
    ...
    const response = await axios.post('http://localhost:8002/generate-model', formData, {
      headers: {
        'Content-Type': 'multipart/form-data'
      }
    })
    ...
    error.value = err.response?.data?.detail || 'Failed to generate model. Please try again.'
  } finally {
