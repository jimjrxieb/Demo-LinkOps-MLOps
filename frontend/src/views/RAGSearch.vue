<template>
  <div class="p-6 animate-slide-in bg-gray-900 text-white min-h-screen">
    <div class="max-w-6xl mx-auto">
      <h2 class="text-3xl font-bold mb-6 text-blue-400">RAG Search: Document Q&A</h2>
      <p class="mb-6 text-gray-300 text-lg">Upload documents and ask questions. Get intelligent answers based on your uploaded content.</p>
      
      <!-- File Upload Section -->
      <div class="bg-gray-800 rounded-lg p-6 mb-6 border border-gray-700">
        <h3 class="text-xl font-semibold mb-4 text-green-400">📄 Upload Documents</h3>
        <div class="space-y-4">
      ...
      <!-- Query Input Section -->
      <div class="bg-gray-800 rounded-lg p-6 mb-6 border border-gray-700">
        <h3 class="text-xl font-semibold mb-4 text-green-400">🔍 Ask Questions</h3>
      ...
      <div v-if="searchResults.length > 0 || llmAnswer" class="bg-gray-800 rounded-lg p-6 border border-gray-700 animate-slide-in">
        <div class="flex items-center justify-between mb-4">
          ...
          <div class="text-sm text-gray-400">
            ...
            <span v-else>
              Found {{ searchResults.length }} results in {{ executionTime.toFixed(2) }}s
            </span>
          </div>
        </div>
        ...
        <div v-if="llmAnswer" class="mb-6">
          <div class="bg-purple-900 rounded-lg p-4 border-l-4 border-purple-500 mb-4">
            ...
            <p class="text-gray-200 leading-relaxed whitespace-pre-wrap">{{ llmAnswer }}</p>
          </div>
          
          <!-- Citations Section -->
          <div v-if="citations && citations.length > 0" class="mt-6">
            <h4 class="text-lg font-medium text-green-400 mb-3">📚 Citations & Source Chunks</h4>
            <div class="space-y-3">
              <div
              ...
              >
                ...
                <div class="bg-gray-600 rounded p-2">
                  <p class="text-gray-200 text-sm leading-relaxed">{{ citation }}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-if="searchResults.length > 0" class="mt-4">
            <h4 class="text-lg font-medium text-blue-400 mb-3">📖 Detailed Source Documents</h4>
            <div class="space-y-3">
              <div
        ...
        <div v-else class="space-y-4">
          ...
          >
            ...
            <div class="bg-gray-600 rounded p-3 mb-3">
              <p class="text-gray-200 leading-relaxed">{{ result.content }}</p>
            </div>
            
            <div v-if="result.metadata" class="text-xs text-gray-400">
              <div v-if="result.metadata.page" class="mb-1">Page: {{ result.metadata.page }}</div>
              <div v-if="result.metadata.chunk_id" class="mb-1">Chunk ID: {{ result.metadata.chunk_id }}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div v-if="queryHistory.length > 0" class="mt-8">
        ...
        <div class="space-y-3">
          <div
            ...
            :class="item.type === 'llm' ? 'border-purple-500' : 'border-blue-500'"
            ...
            <div class="flex justify-between items-start">
              <div>
                <div class="flex items-center space-x-2">
                  ...
                  <span v-if="item.type === 'llm'" class="text-xs bg-purple-600 text-white px-2 py-1 rounded">AI</span>
                </div>
                <p class="text-sm text-gray-400 mt-1">
                  <span v-if="item.type === 'llm'">
                    AI answer using {{ item.model }} ({{ item.citations?.length || 0 }} citations)
                  </span>
                  <span v-else>
      ...
      <div class="mt-8 bg-gray-800 rounded-lg p-4 border border-gray-700">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
          <div class="flex items-center space-x-2">
            <div :class="systemStatus.rag ? 'bg-green-500' : 'bg-red-500'" class="w-2 h-2 rounded-full"></div>
          ...
          <div class="flex items-center space-x-2">
            ...
            <span>Vector Index: {{ systemStatus.vectorstore ? 'Ready' : 'Not Ready' }}</span>
          </div>
          <div class="flex items-center space-x-2">
    ...
    async uploadFiles() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          this.$toast?.success(`Successfully uploaded ${this.selectedFiles.length} files!`);
          this.selectedFiles = [];
          await this.checkSystemStatus();
        } else {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (docsResponse.ok) {
            ...
            this.systemStatus.vectorstore = stats.vectorstore_status === 'ready';
          }
          
          // Test LLM availability
          try {
            const llmResponse = await fetch('http://localhost:9000/rag/query-llm', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ query: 'test', top_k: 1, similarity_threshold: 0.5 })
            });
