...
class RuneStep(BaseEntity):
    ...
    name: str = Field(min_length=1, max_length=100, description="Name of the step")
    description: Optional[str] = Field(
        default=None, max_length=500, description="Description of what the step does"
    )
    ...
    env: Dict[str, str] = Field(
        default_factory=dict, description="Environment variables for the step"
    )
    timeout: int = Field(default=300, ge=1, le=3600, description="Timeout in seconds")
    retries: int = Field(
        default=0, ge=0, le=10, description="Number of retries on failure"
    )
    condition: Optional[str] = Field(
        default=None, description="Condition for step execution"
    )
    on_failure: Optional[str] = Field(
        default=None, description="Action to take on failure"
    )
    on_success: Optional[str] = Field(
        default=None, description="Action to take on success"
    )
...
class Rune(BaseEntity):
    ...
    # Core fields
    name: str = Field(min_length=1, max_length=100, description="Name of the Rune")
    description: Optional[str] = Field(
        default=None, max_length=500, description="Description of what the Rune does"
    )
    type: RuneType = Field(description="Type of the Rune")
    status: RuneStatus = Field(
        default=RuneStatus.DRAFT, description="Status of the Rune"
    )

    # Execution configuration
    steps: List[RuneStep] = Field(default_factory=list, description="Steps to execute")
    timeout: int = Field(
        default=1800, ge=1, le=7200, description="Total timeout in seconds"
    )
    retries: int = Field(
        default=0, ge=0, le=5, description="Number of retries on failure"
    )
    priority: RunePriority = Field(
        default=RunePriority.NORMAL, description="Execution priority"
    )
    ...
    default_input: Optional[Dict[str, Any]] = Field(
        default=None, description="Default input values"
    )

    # Execution tracking
    execution_count: int = Field(
        default=0, ge=0, description="Number of times this Rune has been executed"
    )
    ...
    last_success_at: Optional[datetime] = Field(
        default=None, description="Timestamp of last successful execution"
    )
    last_failure_at: Optional[datetime] = Field(
        default=None, description="Timestamp of last failed execution"
    )
    ...
    min_execution_time: float = Field(
        default=0.0, ge=0.0, description="Minimum execution time in seconds"
    )
    max_execution_time: float = Field(
        default=0.0, ge=0.0, description="Maximum execution time in seconds"
    )

    # Safety and validation
    safe_mode: bool = Field(
        default=True, description="Whether the Rune runs in safe mode"
    )
    dry_run: bool = Field(default=False, description="Whether to perform a dry run")
    validated: bool = Field(
        default=False, description="Whether the Rune has been validated"
    )
    validated_by: Optional[str] = Field(
        default=None, description="User or service that validated the Rune"
    )
    validated_at: Optional[datetime] = Field(
        default=None, description="Timestamp when the Rune was validated"
    )

    # Related resources
    orb_ids: List[str] = Field(default_factory=list, description="IDs of related Orbs")

    # Metadata
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Additional metadata for the Rune"
    )
...
class RuneCreate(BaseEntity):
    ...
    type: RuneType = Field(description="Type of the Rune")
    steps: List[RuneStep] = Field(description="Steps to execute")
    timeout: int = Field(
        default=1800, ge=1, le=7200, description="Total timeout in seconds"
    )
    retries: int = Field(
    ...
    requirements: Dict[str, str] = Field(
        default_factory=dict, description="Requirements for execution"
    )
    input_schema: Optional[Dict[str, Any]] = Field(
        default=None, description="JSON schema for input validation"
    )
...
class RuneUpdate(BaseEntity):
    ...
    name: Optional[str] = Field(
        default=None, min_length=1, max_length=100, description="Name of the Rune"
    )
    description: Optional[str] = Field(
        default=None, max_length=500, description="Description of what the Rune does"
    )
    type: Optional[RuneType] = Field(default=None, description="Type of the Rune")
    status: Optional[RuneStatus] = Field(default=None, description="Status of the Rune")
    steps: Optional[List[RuneStep]] = Field(
        default=None, description="Steps to execute"
    )
    timeout: Optional[int] = Field(
        default=None, ge=1, le=7200, description="Total timeout in seconds"
    )
    retries: Optional[int] = Field(
        default=None, ge=0, le=5, description="Number of retries on failure"
    )
    priority: Optional[RunePriority] = Field(
        default=None, description="Execution priority"
    )
    requirements: Optional[Dict[str, str]] = Field(
        default=None, description="Requirements for execution"
    )
    input_schema: Optional[Dict[str, Any]] = Field(
        default=None, description="JSON schema for input validation"
    )
    ...
    safe_mode: Optional[bool] = Field(
        default=None, description="Whether the Rune runs in safe mode"
    )
...
class RuneExecution(BaseEntity):
    ...
    rune_id: str = Field(description="ID of the Rune being executed")
    status: RuneExecutionStatus = Field(
        default=RuneExecutionStatus.PENDING, description="Execution status"
    )
    input_data: Optional[Dict[str, Any]] = Field(
        default=None, description="Input data for the execution"
    )
    output_data: Optional[Dict[str, Any]] = Field(
        default=None, description="Output data from the execution"
    )
...
def create_rune_from_steps(
    name: str, steps: List[RuneStep], rune_type: RuneType, created_by: str, **kwargs
) -> Rune:
    """Create a new Rune from steps."""
    return Rune(name=name, steps=steps, type=rune_type, created_by=created_by, **kwargs)
