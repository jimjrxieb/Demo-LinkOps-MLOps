...
class WhisSettings(BaseSettings):
    ...
    allowed_extensions: List[str] = Field(default=[".txt", ".md", ".json", ".yaml", ".yml"])
...
class Settings(BaseSettings):
    ...
    def validate_service_name(cls, v):
        if not v.replace("-", "").replace("_", "").isalnum():
            raise ValueError("Service name must be alphanumeric with hyphens or underscores only")
        return v
...
def get_service_settings(service_name: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "security": {k: v.get_secret_value() if hasattr(v, 'get_secret_value') else v 
