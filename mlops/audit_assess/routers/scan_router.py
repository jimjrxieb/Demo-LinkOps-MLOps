...
@router.post("/audit")
async def audit_repo(data: dict = Body(...)):
    """Comprehensive repository audit with security and GitOps compliance"""
    try:
        ...
        report = await analyzer.analyze_repository(repo_url, branch)
        ...
        return {
            ...
            "report": report,
            "summary": {
                "security_score": report.get("security_scan", {})
                .get("summary", {})
                .get("compliance_score", 0),
                "gitops_score": report.get("gitops_compliance", {}).get(
                    "overall_score", 0
                ),
                "total_issues": report.get("security_scan", {})
                .get("summary", {})
                .get("total_issues", 0),
                "grade": report.get("gitops_compliance", {}).get("grade", "F"),
            },
        }
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to audit repository: {str(e)}"
        )
