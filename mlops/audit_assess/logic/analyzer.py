...
class RepoAnalyzer:
    ...
    async def analyze_repository(
        ...
        try:
            ...
            security_scanner = SecurityScanner(self.repo_path)
            security_results = await security_scanner.run_full_security_scan()
            analysis["security_scan"] = security_results

            # Run GitOps compliance scan
            gitops_scanner = GitOpsScanner(self.repo_path)
            gitops_results = await gitops_scanner.scan_gitops_compliance()
            analysis["gitops_compliance"] = gitops_results
    ...
    def _analyze_structure(self) -> Dict[str, Any]:
        ...
        structure = {"root_files": [], "directories": [], "total_files": 0, "depth": 0}
        ...
        for root, dirs, files in os.walk(self.repo_path):
            ...
            structure["depth"] = max(
                structure["depth"], rel_path.count(os.sep) if rel_path != "." else 0
            )
                    ...
                    dockerfiles.append(
                        {
                            "file": rel_path,
                            "type": (
                                "Dockerfile" if file.startswith("Dockerfile") else file
                            ),
                            "content_preview": self._get_file_preview(
    ...
    def _detect_security_issues(self) -> List[Dict[str, Any]]:
        ...
        for root, dirs, files in os.walk(self.repo_path):
            ...
            for file in files:
                if file.endswith(
                    (".py", ".js", ".ts", ".java", ".go", ".yml", ".yaml", ".json")
                ):
                    file_path = os.path.join(root, file)
                    content = self._get_file_preview(file_path)
