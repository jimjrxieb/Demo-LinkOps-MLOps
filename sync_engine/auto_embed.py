...
def process_file(path: Path) -> Dict[str, Any]:
    ...
    try:
        ...
        processing_path = PROCESSING_DIR / f"processing_{start_time.strftime('%Y%m%d_%H%M%S')}_{original_name}"
        ...
        if embed_success:
            ...
            processed_path = PROCESSED_DIR / f"processed_{start_time.strftime('%Y%m%d_%H%M%S')}_{original_name}"
        ...
        failed_path = FAILED_DIR / f"failed_{start_time.strftime('%Y%m%d_%H%M%S')}_{original_name}"
...
def get_processing_stats() -> Dict[str, Any]:
    ...
    try:
        ...
        today_log = LOGS_DIR / f"processing_log_{datetime.now().strftime('%Y%m%d')}.jsonl"
        if today_log.exists():
            with open(today_log, 'r') as f:
                lines = f.readlines()
                stats["today_processed"] = len([l for l in lines if '"status": "completed"' in l])
                stats["today_failed"] = len([l for l in lines if '"status": "failed"' in l])
        else:
